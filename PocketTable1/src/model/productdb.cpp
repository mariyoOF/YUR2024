...
void ProductDB::add_order(string date, string email, vector<Product> products) {
  for (auto product : products) {
    QString query = tr("INSERT INTO products_order(date, email, id_product) VALUES (")
                          + qs(date) + ","
                          + qs(email) + ","
...
vector<Order> ProductDB::orders() {
  ...
  QString query = tr("SELECT date, email, id_product FROM products_order ");
  exec(query);
