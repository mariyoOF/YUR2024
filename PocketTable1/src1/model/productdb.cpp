...
bool ProductDB::is_login_busy(QString login) { //функциия для работы с логинами
...
void ProductDB::add_user(User user) { //функция добавления пользователя
...
vector<User> ProductDB::users() { //функция для работы с данными пользователя
  vector<User> users;
...
User ProductDB::get_user(QString login) { //находим пользователя по логину
...
void ProductDB::remove_user(string login) { //удаляем пользователя
  QString query = tr("DELETE FROM users WHERE login = ") + qs(login);
  exec(query);
  emit updated();
}

void ProductDB::add_product(Product product) { //добавляем товар и его характеристики
...
vector<Product> ProductDB::products() { //для работы с данными товаров
...
bool ProductDB::product_by_article(string article, Product& product) { //ищем товар по артиклю
...
bool ProductDB::product_by_id(int id, Product& product) { //ищем товар по id
  QString query = tr("SELECT name, article, cost, kind, expiry_date, receipt_date, id "
...
void ProductDB::remove_product(string article) { //удаляем данные товара
...
void ProductDB::add_order(string date, vector<Product> products) { //добавляем заказ
...
vector<Order> ProductDB::orders() { //функция для раюоты с заказом
  vector<Order> orders;
  ...
  while (true == m_query->next()) {
    ...
    auto it_order = find_if(orders.begin(), orders.end(), [=](Order order){
        return order.date == date; //&& order.email == email; Разобраться и заменить значение вывода
    });
...
void ProductDB::remove_order(string date) { //удаляем заказ
...
void ProductDB::sell_order(string date) { //одобряем заказ
  ...
  for (auto id : sold_ids) {
    ...
    QString query = tr("DELETE FROM products WHERE id = ") + QString::number(id); //зачистили БД от проданных товаров
    exec(query);
  }
...
int ProductDB::count(QString from, QString to) { //считаем заказ
    exec(
...
int ProductDB::sum(QString from, QString to) { //считаем сумму
    exec(
